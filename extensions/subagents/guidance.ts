...
export function buildActiveAgentsBlock(active: AgentConfig[]): string {
	...
	return [
		...
		"Event triggers are discipline backstops: Known failure, crash, or non-zero test/build exit means use the debugging specialist; about to declare code done or commit means use the review specialist unless the diff is trivial.",
		...
		"Implementation belongs to the worker specialist when the task is a well-scoped edit and not a Svelte file; do not substitute reviewer for code-writing work.",
